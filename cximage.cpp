...
#include "pch.h"
#include "ximage.h"
#pragma comment(lib,"E:/06study/cximage/CxImage/Release/cximage.lib")
#pragma comment(lib,"E:/06study/cximage/jpeg/Release/jpeg.lib")
#pragma comment(lib,"E:/06study/cximage/png/Release/png.lib")
#pragma comment(lib,"E:/06study/cximage/mng/Release/mng.lib")
#pragma comment(lib,"E:/06study/cximage/tiff/Release/tiff.lib")
#pragma comment(lib,"E:/06study/cximage/jasper/Release/jasper.lib")
#pragma comment(lib,"E:/06study/cximage/jbig/Release/jbig.lib")
#pragma comment(lib,"E:/06study/cximage/raw/Release/libdcr.lib")
#pragma comment(lib,"E:/06study/cximage/zlib/Release/zlib.lib")
#pragma comment(lib,"E:/06study/cximage/libpsd/Release/libpsd.lib")
...
#define ENABLE_SHOW_CONVERSION 1


	if (argc < 3) {
		clock_t start, eend;
		start = clock();
		int filesnum = convert_image_types("*.bmp", CXIMAGE_FORMAT_BMP, "*.jpg", CXIMAGE_FORMAT_JPG);
		eend = clock();
		printf(" totalfiles: %d\n total time:%dms\n", filesnum, eend - start);
		getchar();
		return status;
...
int	convert_image_types(TCHAR* input_image, int typein, TCHAR* output_image, int typeout)
{
	...
	find_handle = (long)_tfindfirst(input_image, &c_file);
	if (find_handle > 0)
	{
		do {
			_tcscpy(file_name, c_file.name); // Get the found file name
			file_name[_tcsrchr(c_file.name, '.') - c_file.name + 1] = 0; // cut to the file extention
			_tcscat(file_name, (TCHAR*)(_tcschr(output_image, '.') + 1));
#if  ENABLE_SHOW_CONVERSION
